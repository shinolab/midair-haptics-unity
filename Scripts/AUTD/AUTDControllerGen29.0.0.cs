...
public class AUTDControllerGen29_0_0<T> : AUTDController
{
    public Controller<T>? _autd = null;
    public bool visualizeDevices = false;
    private Duration time = Duration.FromMillis(0);
    ...
    public static new AUTDControllerGen29_0_0<T> Instance
    {
        get
        {
            if (AUTDController.Instance is AUTDControllerGen29_0_0<T> childInstance)
            {
    ...
    public override void Silence(int step)
    {
        ...
        stepSilencer = step;
        stepAmplitude = 65535;
        _autd?.Send(config.WithTimeout(Duration.FromMillis(100)));
    }
    ...
    public override void AM(int frequency)
    {
        ...
        if (frequency == 0)
            ...
            _autd?.Send(new AUTD3Sharp.Modulation.Sine(frequency * Hz).WithTimeout(Duration.FromMillis(100)));
    }
    ...
    public override void AMFourier(List<float> frequency, List<float> amplitude, List<float> offset)
    {
        ...
        _autd?.Send(m.WithTimeout(Duration.FromMillis(0)));
    }
    ...
    public override void Static(float amp = 1, float timeSpan = 100)
    {
        ...
        byte bamp = (byte)iamp;
        _autd?.Send(AUTD3Sharp.Modulation.Static.WithIntensity(bamp).WithTimeout(Duration.FromMillis((ulong)timeSpan)));
    ...
    public override void MultiFocus(List<Vector3> pos, float amp = 1f)
    {
        ...
        {
            var vec = new AUTD3Sharp.Utils.Point3(p.x, p.y, p.z);
    ...
    public override void MultiFocus(List<Vector3> pos, List<float> amp)
    {
        ...
        var g = new Naive(backend, list).WithConstraint(EmissionConstraint.Clamp(EmitIntensity.Min, EmitIntensity.Max));
    ...
    public override void MultiFocusMaskBasedOnNormal(List<Vector3> pos, float amp, Vector3 point, Vector3 normal, float thresholdMasking)
    {
       ...
       var group = new AUTD3Sharp.Gain.Group(dev => tr =>
        ...
        var g = new Naive(backend, list).WithConstraint(EmissionConstraint.Uniform(bamp));
    ...
    public override void MultiFocusMaskBasedOnNormal(List<Vector3> pos, List<float> amp, Vector3 point, Vector3 normal, float thresholdMasking) {
    ...
    public override void FocusSTM(List<Vector3> pos, float frequency, List<byte>? byteShift = null)
    {
        ...
        for (int i = 0; i < pos.Count; i++)
        {
            ...
            list.Add(new ControlPoints1(new AUTD3Sharp.Utils.Point3(vec.x, vec.y, vec.z)));
        ...
        var stm = FociSTM.Nearest(frequency * Hz, list);
    ...
    public override void FocusSTM(List<Vector3> pos, float frequency, long timespan)
    {
        ...
        var stm = FociSTM.Nearest(frequency * Hz, list);
        _autd?.Send(stm.WithTimeout(Duration.FromMillis((ulong)timespan)));
    ...
    public override void MultiFocusSTM(List<List<Vector3>> pos, float frequency, long timespan)
    {
        ...
        switch (pos[0].Count)
        {
            case 8:
                ...
                {
                    list.Add(new ControlPoints8((new AUTD3Sharp.Utils.Point3(points[i][0].x, points[i][0].y, points[i][0].z),
                        new AUTD3Sharp.Utils.Point3(points[i][1].x, points[i][1].y, points[i][1].z),
                        new AUTD3Sharp.Utils.Point3(points[i][2].x, points[i][2].y, points[i][2].z),
                        new AUTD3Sharp.Utils.Point3(points[i][3].x, points[i][3].y, points[i][3].z),
                        new AUTD3Sharp.Utils.Point3(points[i][4].x, points[i][4].y, points[i][4].z),
                        new AUTD3Sharp.Utils.Point3(points[i][5].x, points[i][5].y, points[i][5].z),
                        new AUTD3Sharp.Utils.Point3(points[i][6].x, points[i][6].y, points[i][6].z),
                        new AUTD3Sharp.Utils.Point3(points[i][7].x, points[i][7].y, points[i][7].z))));
                }
    ...
    {
        _autd?.Send(new AUTD3Sharp.Gain.Focus(pos / scaleUnity).WithTimeout(Duration.FromMillis((ulong)timeOut)));
    }
        ...
        for (int i = 0; i < idDevice.Count; i++)
        {
            ...
            else
            {
                ...
                for (int j = 0; j < pos[i].Count; j++)
                {
                    ...
                    stmPos.Add(new ControlPoints1(new AUTD3Sharp.Utils.Point3(vec.x, vec.y, vec.z)));
                }

                var stm = FociSTM.Nearest(frequency[i] * Hz, stmPos);
        ...
        for (int i = 0; i < idDevice.Count; i++)
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    var multi = new Naive(backend, list).WithConstraint(EmissionConstraint.Uniform(bamp));
